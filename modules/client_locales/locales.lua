...
local translationWarnings = {}  -- Cache para evitar spam de avisos repetidos
function _G.tr(text, ...)
    if currentLocale then
            ...
            if not translation then
                if translation == nil then
                    if currentLocale.name ~= defaultLocaleName then
                        -- CORRECAO: Limitar spam de avisos de traducao em modo DEBUG
                        local textToShow = text
                        -- Truncar textos muito longos (> 50 caracteres)
                        if #text > 50 then
                            textToShow = text:sub(1, 50) .. "... (truncated)"
                        end
                        -- Apenas logar se ainda nao foi avisado (evita spam)
                        if not translationWarnings[text] then
                            pdebug('Unable to translate: \"' .. textToShow .. '\"')
                            translationWarnings[text] = true
                        end
                    end
                end
                translation = text
