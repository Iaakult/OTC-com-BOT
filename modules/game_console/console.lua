...
function clear()
    ...
    -- Remover aba de Loot se existir
    if lootTab then
...
function removeTab(tab)
    ...
    elseif tab.lootChannel then
        -- Fechar aba de Loot (apenas fecha localmente, nao envia nada ao servidor)
        lootTab = nil
    end
...
function doChannelListSubmit()
    ...
    if openPrivateChannelWith ~= '' then
        ...
        -- CORRECAO: Tratar canal "Loot" (ID -1)
        if selectedChannelLabel.channelId == -1 then
            if not lootTab then
                lootTab = addTab(tr('Loot'), true)
                lootTab.lootChannel = true
            else
...
function onChannelList(channelList)
    ...
    -- CORRECAO: Adicionar opcao "Loot" na lista de canais
    local lootLabel = g_ui.createWidget('ChannelListLabel', channelListPanel)
    lootLabel.channelId = -1 -- ID especial para canal local
    lootLabel:setText('Loot')
    lootLabel:setPhantom(false)
    lootLabel.onDoubleClick = function()
        if not lootTab then
            lootTab = addTab(tr('Loot'), true)
            lootTab.lootChannel = true
        else
...
function online()
    ...
    -- Criar aba de Loot (pseudo-canal local, igual ao NPCs)
