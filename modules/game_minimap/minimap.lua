...
function mapController:onGameStart()
    ...
    -- CORREÇÃO CRÍTICA: Verificar AppData primeiro, só limpar se necessário
    local minimapFile = '/minimap'
    local userMinimapFile = nil
    local loadFnc = nil
    local loaded = false

    if otmm then
        minimapFile = minimapFile .. '.otmm'
        userMinimapFile = g_resources.getUserDir() .. 'minimap.otmm'
        loadFnc = g_minimap.loadOtmm
    else
        minimapFile = minimapFile .. '_' .. g_game.getClientVersion() .. '.otcm'
        userMinimapFile = g_resources.getUserDir() .. 'minimap_' .. g_game.getClientVersion() .. '.otcm'
        loadFnc = g_map.loadOtcm
    end

    -- Primeiro: tentar carregar do AppData (suas explorações)
    if g_resources.fileExists(userMinimapFile) then
        loaded = loadFnc(userMinimapFile)
    end
    
    -- Se não conseguiu carregar do AppData, carregar original
    if not loaded then
        g_minimap.clean()
...
function mapController:onGameEnd()
    -- Save Map - usar caminho relativo simples (salva no AppData automaticamente)
    if otmm then
