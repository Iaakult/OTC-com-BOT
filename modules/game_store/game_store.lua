...
-- == Enums ==--
GameStore.website = {
    WEBSITE_GETCOINS = "https://lattorre.com/?donatepaypal",
    IMAGES_URL =  "http://172.86.113.137/images/store/" --./game_store --https://docs.opentibiabr.com/opentibiabr/downloads/website-applications/applications#store-for-client-13-1
}
...
local function createProductImage(imageParent, data)
    ...
    elseif data.VALOR == "mountId" or data.VALOR:find("outfitId") then
        ...
        creature:setOutfit({
            type = data.ID,
            addons = 3  -- Mostra addons 1 e 2 completos (1 + 2 = 3)
        })
...
function onParseStoreGetPurchaseStatus(purchaseStatus)
    ...
    -- Verificar se buttonAnimation existe antes de acessar
    if messageBox.buttonAnimation and messageBox.buttonAnimation.animation then
        ...
        -- Se não houver buttonAnimation, criar um botão simples de fechar
        g_logger.warning("buttonAnimation not found in confirmarSHOP widget, using fallback")
        addEvent(function()
            destroyWindow({messageBox})
            controllerShop.ui:show()
            fixServerNoSend0xF2()
