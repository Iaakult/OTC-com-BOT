...
function StatsBar.onUpdateProficiencyData(itemCache, hasUnnusedPerk, thingType)
    ...
    if not proficiencyModule or not proficiencyModule.ProficiencyData then
        return
    end
    ...
    local maxAvailableLevel = proficiencyModule.ProficiencyData:getPerkLaneCount(thingType:getProficiencyId()) + 2
    local weaponLevel = proficiencyModule.ProficiencyData:getCurrentLevelByExp(thingType, itemCache.exp, true)
    local percent = proficiencyModule.ProficiencyData:getLevelPercent(itemCache.exp, math.min(maxAvailableLevel, weaponLevel + 1), thingType)
    local maxLevelExperience = proficiencyModule.ProficiencyData:getMaxExperienceByLevel(math.min(maxAvailableLevel, weaponLevel + 1), thingType)
