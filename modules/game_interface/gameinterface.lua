...
function forceExit()
    g_game.cancelLogin()
    -- CORRECAO: Aumentar delay para 500ms para dar tempo do servidor processar o logout
    -- Isso evita "Connection reset by peer" no servidor
    scheduleEvent(exit, 500)
    return true
end
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
    ...
    if lookThing then
        ...
        --
                    if lookThing:getProficiencyId() > 0 and modules.game_proficiency and modules.game_proficiency.requestOpenWindow then
