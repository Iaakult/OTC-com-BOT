...
function init()
    ...
    -- CORRECAO: Adicionar handler para fechar janela (X ou Alt+F4)
    -- Envia logout antes de fechar para evitar "Connection reset by peer"
    g_app.onClose = function()
        if g_game.isOnline() then
            g_game.safeLogout()
            -- Aguardar 300ms para o pacote de logout chegar ao servidor
            scheduleEvent(function()
                g_app.exit()
            end, 300)
            return true -- Cancela o fechamento imediato
        end
        return false -- Permite fechar normalmente se nao estiver online
    end
