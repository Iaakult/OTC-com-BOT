...
-- character 14.10
            ...
            -- houses
            onParseHouseList = function(...)
                print("=== onParseHouseList CALLED ===")
                print("Arguments:", ...)
                return Cyclopedia.loadHouseList(...)
            end,
            onCyclopediaHouseList = function(houses)
                -- Processar dados recebidos do Canary
                local mockData = {}
                local mockOther = {}
                ...
                for _, houseData in ipairs(houses) do
                    -- Desempacotar tupla do C++
                    local clientId, state, ownerOrBidder, paidUntilOrBidEnd, highestBid, bidHolderLimit, renovationOrAvailable = unpack(houseData)
                    
                    -- CONVERTER clientId para houseid do banco (a tabela HOUSE usa houseid)
                    local houseid = HOUSE_CLIENTID_MAP and HOUSE_CLIENTID_MAP[clientId] or clientId
                    
                    -- Buscar informações da house na tabela HOUSE
                    local houseInfo = HOUSE[houseid]
                    
                    -- Se não encontrou, pode ser que clientId == houseid (houses antigas)
                    if not houseInfo and clientId ~= houseid then
                        houseInfo = HOUSE[clientId]
                        houseid = clientId
                    end
                    
                    -- Se ainda não encontrou, pular essa house
                    if not houseInfo then
                        -- Log silencioso, não precisa poluir o console
                    else
                        local houseName = houseInfo.name
                        local houseSize = houseInfo.sqm
                        local houseRent = houseInfo.rent
                        local houseBeds = houseInfo.beds
                        local houseTownId = houseInfo.townid
                        
                        -- Filtrar por cidade se necessario
                        local shouldInclude = true
                        
                        -- Se townId = 0, mostrar APENAS houses do player
                        if Cyclopedia.House.lastTownId == 0 then
                            shouldInclude = (ownerOrBidder == playerName)
                        -- Se townId > 0, filtrar por cidade
                        elseif Cyclopedia.House.lastTownId and Cyclopedia.House.lastTownId > 0 then
                            shouldInclude = (houseTownId == Cyclopedia.House.lastTownId)
                        end
                        ...
                        if shouldInclude then
                            table.insert(mockData, {
                                ...
                                townid = houseTownId,
                                state = state,
                                bidEnd = (state == 0) and paidUntilOrBidEnd or 0,
                                hightestBid = highestBid or 0,
                                bidHolderLimit = bidHolderLimit or 0,
                                selfCanBid = true,
                                paidUntil = (state == 1 or state == 2) and paidUntilOrBidEnd or 0,
                                time = 0,
                                transferValue = 0,
                                hasTransferOwner = 0,
                                canAcceptTransfer = 0
                            ...
                            table.insert(mockOther, {
                                state = state,
                                owner = (state == 1 or state == 2) and ownerOrBidder or "",
                                ownerName = (state == 1 or state == 2) and ownerOrBidder or "",
                                bidName = (state == 0) and ownerOrBidder or "",
                                transferPlayer = ""
                            })
                        end
                    end
                ...
                -- Atualizar UI
                return Cyclopedia.loadHouseList(mockData, mockOther)
            end,
            onCyclopediaHouseAuctionResult = function(...)
                print("=== onCyclopediaHouseAuctionResult CALLED ===")
                print("Arguments:", ...)
                return Cyclopedia.houseMessage(...)
            end
        })
...
function show(defaultWindow)
    ...
    Cyclopedia.updateGoldBalance()
end

function Cyclopedia.updateGoldBalance()
    if controllerCyclopedia and controllerCyclopedia.ui and controllerCyclopedia.ui.GoldBase then
        local player = g_game.getLocalPlayer()
        if player then
            local balance = player:getResourceBalance()
            controllerCyclopedia.ui.GoldBase.Value:setText(Cyclopedia.formatGold(balance))
        end
    end
end
...
function SelectWindow(type, isBackButtonPress)
    ...
    -- Atualizar saldo ao mudar de janela
    Cyclopedia.updateGoldBalance()
end

function onCyclopediaHouseAuctionResult(houseId, typeValue, resultCode)
    if typeValue == 1 then -- Bid
        -- BidSuccessMessage enum no Canary:
        -- BidSuccess = 0x00 (0)
        -- LowerBid = 0x01 (1)
        if resultCode == 0 or resultCode == 1 then
            if resultCode == 0 then
                displayInfoBox(tr("Success"), tr("Bid placed successfully! You are now the highest bidder."))
            else
                displayInfoBox(tr("Success"), tr("Bid placed! But there is a higher bid."))
            end
            
            -- Atualizar saldo após bid bem-sucedido
            scheduleEvent(function()
                Cyclopedia.updateGoldBalance()
            end, 500)
        else
            -- BidErrorMessage codes
            local errorMessages = {
                [3] = "You must be promoted to bid on houses.",
                [5] = "You must have a premium account.",
                [6] = "This is a guild hall. Only guilds can bid.",
                [7] = "You can only bid on one house at a time.",
                [17] = "Not enough gold in bank.",
                [21] = "Not enough gold in guild bank.",
                [24] = "Internal error. Please try again."
            }
            local errorMsg = errorMessages[resultCode] or "Failed to place bid. Error code: " .. resultCode
            displayInfoBox(tr("Error"), tr(errorMsg))
        end
    end
    
    -- NÃO RECARREGAR MANUALMENTE - O Canary já envia a lista atualizada automaticamente!
end
