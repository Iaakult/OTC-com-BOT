...
function Cyclopedia.loadBestiarySelectedCreature(data)
    -- Guard clause: Check if UI is initialized
    if not UI then
        return
    end
...
function Cyclopedia.BestiarySearch()
    ...
    if #raceList > 0 then
        -- Create a search result data structure similar to category creatures
        local searchResults = {}
        for _, race in ipairs(raceList) do
            -- Request bestiary data from server for this race to get currentLevel
            local raceData = {
                id = race.raceId,
                currentLevel = 1, -- Default, will be updated by server response
                AnimusMasteryBonus = 0
            }
            table.insert(searchResults, raceData)
        end
        
        -- Load search results using the existing function
        Cyclopedia.loadBestiarySearchCreatures(searchResults)
    end
