...
function showHouse()
    ...
    -- Sempre popular os filtros (UI é recriada toda vez)
...
-- Function to load house data from website API
function Cyclopedia.loadMockHouseData(townName, townid)
    -- Store for callback
    Cyclopedia.House.lastTown = townName
    Cyclopedia.House.lastTownId = townid
    
    -- Request houses from server via PROTOCOL
    g_game.sendCyclopediaHousesByTown(townName or "")
end

-- Fallback function with mock data (if API fails)
function Cyclopedia.loadMockHouseDataFallback(townName, townid)
    local mockData = {}
    local mockOther = {}
    
    -- Create mock data for all houses in HOUSE table
    for id, houseInfo in pairs(HOUSE) do
        -- Filter by town ID
        local shouldInclude = true
        if townid and townid > 0 then
            shouldInclude = (houseInfo.townid == townid)
        end
        
        if shouldInclude then
            table.insert(mockData, {
                houseId = id,
                state = 1, -- 1=auctioned (available for bidding)
                bidEnd = os.time() + math.random(3600, 86400),
                hightestBid = math.random(5000, 50000),
                bidHolderLimit = 0,
                selfCanBid = 0,
                paidUntil = os.time() + math.random(86400, 2592000),
                time = 0,
                transferValue = 0,
                hasTransferOwner = 0,
                canAcceptTransfer = 0
            ...
            table.insert(mockOther, {
                owner = "Player " .. math.random(1, 100),
                bidName = "Bidder " .. math.random(1, 50),
                transferPlayer = nil
            })
        end
    end
    
    -- Call the actual load function with mock data
    if #mockData > 0 then
        Cyclopedia.loadHouseList(mockData, mockOther)
    else
        Cyclopedia.loadHouseList({}, {})
    end
end

Cyclopedia.CityList = {
    [0] = { Title = "Own Houses", townid = 0 },
    { Title = "Ab'Dendriel", townid = 5 },
    { Title = "Ankrahmun", townid = 10 },
    { Title = "Candia", townid = 29 },
    { Title = "Carlin", townid = 6 },
    { Title = "Darashia", townid = 13 },
    { Title = "Edron", townid = 11 },
    { Title = "Farmine", townid = 12 },
    { Title = "Gray Beach", townid = 18 },
    { Title = "Issavi", townid = 22 },
    { Title = "Kazordoon", townid = 7 },
    { Title = "Liberty Bay", townid = 14 },
    { Title = "Moonfall", townid = 41 },
    { Title = "Port Hope", townid = 15 },
    { Title = "Rathleton", townid = 20 },
    { Title = "Silvertides", townid = 40 },
    { Title = "Svargrond", townid = 16 },
    { Title = "Thais", townid = 8 },
    { Title = "Venore", townid = 9 },
    { Title = "Yalahar", townid = 17 }
}
...
function Cyclopedia.houseMessage(houseId, type, message)
    ...
    local function yesCallback()
        if confirmWindow then
            ...
            -- Reload house list to show updated information
...
function Cyclopedia.rejectTransfer()
    ...
    function UI.rejectTransferHouse.transfer:onClick()
        ...
        local function yesCallback()
            ...
            -- Send REJECT TRANSFER to server (Type: 7 = REJECT_TRANSFER)
            g_game.sendCyclopediaHouseAuction(7, house.id, 0, 0, "")
...
function Cyclopedia.acceptTransfer()
    ...
    function UI.acceptTransferHouse.transfer:onClick()
        ...
        local function yesCallback()
            ...
            -- Send ACCEPT TRANSFER to server (Type: 6 = ACCEPT_TRANSFER)
            g_game.sendCyclopediaHouseAuction(6, house.id, 0, 0, "")
            Cyclopedia.House.ignore = true

            -- g_game.requestShowHouses("")
            Cyclopedia.loadMockHouseData("")
            UI.TopBase.StatesOption:setOption("All States", true)
            UI.TopBase.SortOption:setOption("Sort by name", true)
        end
...
function Cyclopedia.cancelTransfer()
    ...
    function UI.cancelHouseTransferArea.transfer:onClick()
        ...
        local function yesCallback()
            ...
            -- Send CANCEL TRANSFER to server (Type: 5 = CANCEL_TRANSFER)
            g_game.sendCyclopediaHouseAuction(5, house.id, 0, 0, "")
...
function Cyclopedia.transferHouse()
    ...
    local function verify(widget, text, type)
        -- Verificar se os dropdowns têm opções selecionadas
        ...
        if not yearOption or not monthOption or not dayOption then
            return -- Dropdowns ainda não inicializados
        end
        
        local timestemp = os.time({
            year = yearOption.text,
            month = monthOption.text,
            day = dayOption.text
        })
    ...
    function UI.transferArea.transfer:onClick()
        ...
        local function yesCallback()
            ...
            -- Send TRANSFER to server (Type: 3 = TRANSFER)
            -- Parameters: houseId, timestamp, bidValue (transfer price), name (new owner)
            g_game.sendCyclopediaHouseAuction(3, house.id, timestemp, tonumber(value), transfer)
...
function Cyclopedia.moveOutHouse()
    ...
    function UI.moveOutArea.move:onClick()
        ...
        local function yesCallback()
            ...
            -- Send MOVE OUT to server (Type: 2 = MOVEOUT)
            -- Parameters: houseId, timestamp (moveout date), bidValue (0), name (empty)
            g_game.sendCyclopediaHouseAuction(2, house.id, timestemp, 0, "")
        end
...
function Cyclopedia.bidHouse(widget)
    ...
    local minimumBid = house.rent or 0
    if house.hightestBid and house.hightestBid > 0 then
        minimumBid = house.hightestBid + 1
    end
    
    if house.bidHolderLimit and house.bidHolderLimit > 0 then
        bidArea.textEdit:setText(house.bidHolderLimit)
    else
        bidArea.textEdit:setText(minimumBid)
    end
    ...
    function bidArea.textEdit:onTextChange(text, oldText)
        ...
        if text == "" or not convertedText or convertedText <= 0 then
    ...
    -- Enable button initially if there's a valid value
    local initialValue = tonumber(bidArea.textEdit:getText())
    ...
    function UI.bidArea.bid:onClick()
        ...
        local function yesCallback()
            ...
            -- Send bid directly to Canary server via protocol
            g_game.sendCyclopediaHouseAuction(1, house.id, 0, value, "")
            
            -- O callback onCyclopediaHouseAuctionResult vai processar a resposta
            -- E o Canary vai enviar a lista de houses atualizada via protocolo
            
            -- Close bid area and show list again
            UI.bidArea:setVisible(false)
            UI.ListBase:setVisible(true)
        end
...
function Cyclopedia.houseRefresh()
    if Cyclopedia.House.lastTown then
        -- g_game.requestShowHouses(Cyclopedia.House.lastTown) -- Function not implemented yet
        Cyclopedia.loadMockHouseData(Cyclopedia.House.lastTown)
...
function Cyclopedia.reloadHouseList()
    if not UI or not UI.LateralBase then
        return
    end
    
    -- Atualizar contador
    local visibleCount = 0
    for _, data in ipairs(Cyclopedia.House.Data or {}) do
        if data.visible then
            visibleCount = visibleCount + 1
        end
    end
    
    if UI.TopBase and UI.TopBase.HouseCounter then
        UI.TopBase.HouseCounter:setText(string.format("Houses: %d", visibleCount))
    end
                ...
                if data.state == 0 then
                    if data.hasBid then
                        ...
                        widget.status:setColoredText("{Status:  , #909090}{auctioned, #00F000} (Bid: " ..
                                                         comma_value(data.hightestBid or 0) .. " Ends in: " .. format(data.bidEnd) .. ")")
                    else
...
function Cyclopedia.loadHouseList(data, other)
    ...
    if not table.empty(data) then
        for i = 1, #data do
            ...
            -- Se house nao existe na tabela HOUSE, usar dados do servidor
            if not house then
                house = {
                    name = value.name or ("House #" .. value.houseId),
                    description = "",
                    rent = value.rent or 0,
                    beds = value.beds or 0,
                    sqm = value.size or 0,
                    GH = 0,
                    shop = 0
                }
            ...
            local data_t = {
                ...
                rent = house.rent,
                beds = house.beds,
                sqm = house.sqm or house.size or value.size or 0,
                gh = isGuildHall,
                ...
                hasBid = (data[i].hightestBid and data[i].hightestBid > 0) or (other[i].bidName and other[i].bidName ~= "" and other[i].bidName ~= "Highest Bidder"),
                ...
                canBid = tonumber(data[i].selfCanBid) or 0,
...
function Cyclopedia.selectTown(widget, text, type)
    ...
    -- Get townid from CityList
    if type and Cyclopedia.CityList[type] then
        townid = Cyclopedia.CityList[type].townid or 0
    end
    
    if type ~= 0 then
        -- g_game.requestShowHouses(name) -- Function not implemented in C++ client yet
        Cyclopedia.House.lastTown = name
        -- Load mock data for now
        Cyclopedia.loadMockHouseData(name, townid)
    else
        -- g_game.requestShowHouses("")
        Cyclopedia.House.lastTown = ""
        -- Load all houses
        Cyclopedia.loadMockHouseData("", 0)
    end
end
...
function Cyclopedia.selectHouse(widget)
    ...
    if widget.data.hasBid then
        ...
        UI.LateralBase.AuctionText:setColoredText("{Hightest Bidder: , #909090}" .. (widget.data.bidName or "No bidder yet") ..
                                                      "\n{      End Time: , #909090}" .. date ..
                                                      "\n{   Highest Bid: , #909090}")
        UI.LateralBase.highestBid:setVisible(true)
        UI.LateralBase.highestBidGold:setVisible(true)
        UI.LateralBase.highestBid:setText(comma_value(widget.data.hightestBid or 0))
    ...
    elseif widget.data.isTransferOwner then
        ...
        -- Convert canBid to number if needed
        local canBidValue = tonumber(widget.data.canBid)
        if canBidValue == nil then
            canBidValue = 0 -- Default to allow bidding if value is invalid
        end

        if canBidValue == 0 then
            button:setEnabled(true)
            button:setTooltip("")
        elseif canBidValue == 11 then
            button:setTooltip(
    ...
    -- Tentar carregar imagem PNG estática primeiro
    local clientId = widget.data.id
    
    -- Converter clientId para houseid do banco (se necessário)
    local houseid = HOUSE_CLIENTID_MAP and HOUSE_CLIENTID_MAP[clientId] or clientId
    ...
    if g_resources.fileExists(imagePath) then
        ...
        -- Se não há imagem PNG, tentar usar minimap dinâmico
        Cyclopedia.setupHouseMinimap(houseid)
    end

    Cyclopedia.House.lastSelectedHouse = widget
end

-- Função para configurar minimap da house dinamicamente
function Cyclopedia.setupHouseMinimap(houseId)
    local minimap = UI.LateralBase.MapViewbase.houseImage
    local houseInfo = HOUSE[houseId]
    
    if not minimap then
        g_logger.error("[Cyclopedia] Minimap widget not found!")
        return
    end
    
    if not houseInfo then
        g_logger.error("[Cyclopedia] House " .. houseId .. " not found in HOUSE table")
        return
    end
    
    -- Verificar se a house tem coordenadas de entrada
    if houseInfo.entry and #houseInfo.entry >= 3 then
        local pos = {x = houseInfo.entry[1], y = houseInfo.entry[2], z = houseInfo.entry[3]}
        
        -- Carregar o minimap se necessário
        minimap:load()
        ...
        g_logger.warning("[Cyclopedia] House " .. houseId .. " has no entry coordinates")
        g_logger.warning("[Cyclopedia] HouseInfo: name=" .. (houseInfo.name or "nil"))
    end
end
