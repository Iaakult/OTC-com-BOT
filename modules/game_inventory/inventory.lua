...
-- ===== Blessings icon logic =====
local blessIconUpdateEvent = nil

local function setBlessingsIconVariant(variant)
  -- variant: 'grey' | 'green' | 'gold'
  local uiOn  = inventoryController.ui.onPanel
  local uiOff = inventoryController.ui.offPanel
  local path  = '/images/inventory/button_blessings_' .. variant
  
  if uiOn and uiOn.blessings then
    uiOn.blessings:setImageSource(path)
  end
  if uiOff and uiOff.blessings then
    uiOff.blessings:setImageSource(path)
  end
end

-- Processa dados reais do onUpdateBlessDialog
local function refreshBlessingsIcon(data)
  if not data or not data.blesses then
    return
  end
  
  local activeBlessings = 0
  
  -- Contar blessings ativas
  for i, entry in ipairs(data.blesses) do
    local totalCount = entry.playerBlessCount + entry.store
    if totalCount >= 1 then
      activeBlessings = activeBlessings + 1
    end
  end
  
  -- Aplicar lógica das cores
  if activeBlessings == 0 then
    setBlessingsIconVariant('grey')
  elseif activeBlessings >= 8 then
    setBlessingsIconVariant('green')
  else
    setBlessingsIconVariant('gold')
  end
end 
...
function inventoryController:onGameStart()
    ...
    inventoryController:registerEvents(g_game, {
        ...
        onUpdateBlessDialog = function(data)
            refreshBlessingsIcon(data)
        end
    }):execute()
    ...
    -- Inicialização do ícone de blessings (cinza por padrão)
    setBlessingsIconVariant('grey')
    
    -- Timer para solicitar dados de blessings periodicamente
    if blessIconUpdateEvent then
        removeEvent(blessIconUpdateEvent)
    end
    blessIconUpdateEvent = cycleEvent(function()
        if g_game.isOnline() then
            g_game.requestBless()
        end
    end, 5000) -- solicita dados a cada 5 segundos
...
function inventoryController:onGameEnd()
    ...
    -- Cancelar timer de blessings e voltar ao cinza
    if blessIconUpdateEvent then
        removeEvent(blessIconUpdateEvent)
        blessIconUpdateEvent = nil
    end
    setBlessingsIconVariant('grey')
...
-- CORRECAO: Funcao necessaria para o modulo game_proficiency
