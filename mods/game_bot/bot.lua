...
function refresh()
  ...
  -- SISTEMA DE VOCACAO - Sistema automatico de profiles por vocacao
  if not settings.vocationConfig then
    settings.vocationConfig = {
      enabled = true,
      mapping = {
        -- Vocacoes basicas
        [1] = 1,   -- Knight -> Profile 1
        [2] = 4,   -- Paladin -> Profile 4
        [3] = 2,   -- Sorcerer -> Profile 2
        [4] = 3,   -- Druid -> Profile 3
        [5] = 5,   -- Monk -> Profile 5
        -- Vocacoes promoted
        [11] = 1,  -- Elite Knight -> Profile 1
        [12] = 4,  -- Royal Paladin -> Profile 4
        [13] = 2,  -- Master Sorcerer -> Profile 2
        [14] = 3,  -- Elder Druid -> Profile 3
        [15] = 5,  -- Exalted Monk -> Profile 5
        -- Default
        [0] = 1    -- None/Default -> Profile 1
      }
    }
  end
  ...
  if settings[index] == nil then
    settings[index] = {
      enabled=false,
      config="",
      autoVocationProfile = true
    }
  end

  -- Carregamento automatico por vocacao
  if settings[index].autoVocationProfile and settings.vocationConfig.enabled then
    local player = g_game.getLocalPlayer()
    if player then
      local vocation = player:getVocation()
      local targetProfile = settings.vocationConfig.mapping[vocation] or 1
      local currentProfile = g_settings.getNumber('profile')
      
      if targetProfile ~= currentProfile then
        g_settings.set('profile', targetProfile)
        ...
        local vocationName = vocationNames[vocation] or "Unknown"
        print("[AutoProfile] " .. vocationName .. " detectado - Profile " .. targetProfile .. " carregado automaticamente")
