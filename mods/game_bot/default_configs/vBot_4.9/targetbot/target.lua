...
-- SISTEMA DE VOCAÇÕES - MELHORADO COM AUTO-ATIVAÇÃO
local vocationSuffixes = {"_ek", "_ms", "_ed", "_rp", "_mk"}
local vocationNames = {"EK", "MS", "ED", "RP", "MK"}
local lastPlayerName = ""

local function getVocationProfile()
  local player = g_game.getLocalPlayer()
  if player then
    local vocation = player:getVocation()
    if vocation == 1 or vocation == 11 then return 1 -- Knight/Elite Knight
    elseif vocation == 3 or vocation == 13 then return 2 -- Sorcerer/Master Sorcerer
    elseif vocation == 4 or vocation == 14 then return 3 -- Druid/Elder Druid
    elseif vocation == 2 or vocation == 12 then return 4 -- Paladin/Royal Paladin
    elseif vocation == 5 or vocation == 15 then return 5 -- Monk/Exalted Monk
    end
  end
  return 1
end

-- Função para encontrar perfil por vocação
local function findProfileByVocation()
  local vocationId = getVocationProfile()
  local suffix = vocationSuffixes[vocationId]
  local botConfigName = modules.game_bot.contentsPanel.config:getCurrentOption().text
  local configDir = "/bot/" .. botConfigName .. "/targetbot_configs/"
  
  -- Procurar por arquivos que terminam com o sufixo da vocação
  local files = g_resources.listDirectoryFiles(configDir, false, false)
  for _, file in ipairs(files) do
    if file:find("%.json$") then
      local baseName = file:gsub("%.json$", "")
      if baseName:find(suffix .. "$") then
        return baseName
      end
    end
  end
...
-- Função para ativar o bot geral
local function activateBot()
  if modules.game_bot and modules.game_bot.contentsPanel and modules.game_bot.contentsPanel.config then
    ...
    if configWidget.setOn then
      configWidget:setOn()
      print("[TargetBot] Bot geral ativado automaticamente")
    end
  end
end

-- Função para selecionar perfil automaticamente
local function autoSelectProfile()
  local player = g_game.getLocalPlayer()
  if not player then return end
  
  local currentPlayerName = player:getName()
  local playerChanged = (lastPlayerName ~= currentPlayerName)
  lastPlayerName = currentPlayerName
  
  local profileName = findProfileByVocation()
  if profileName then
    local currentProfile = storage._configs.targetbot_configs.selected
    if currentProfile ~= profileName then
      print("[TargetBot] Auto-selecionando perfil: " .. profileName .. " para " .. vocationNames[getVocationProfile()])
      storage._configs.targetbot_configs.selected = profileName
      
      -- Se mudou de player, ativar bot automaticamente
    ...
    elseif playerChanged then
      -- Mesmo perfil, mas player diferente - ativar bot
      schedule(1000, function()
        activateBot()
      end)
    end
  else
    print("[TargetBot] Nenhum perfil encontrado para " .. vocationNames[getVocationProfile()] .. ". Crie um perfil terminado em " .. vocationSuffixes[getVocationProfile()])
  end
end
...
-- Executar auto-seleção após 3 segundos e depois a cada 5 segundos
schedule(3000, function() 
  autoSelectProfile()
end)

-- Verificar mudança de player periodicamente
macro(5000, function()
  autoSelectProfile()
end)
...
-- FUNÇÕES PÚBLICAS PARA VOCAÇÕES
TargetBot.getCurrentVocation = function()
  return getVocationProfile()
end

TargetBot.getVocationName = function()
  return vocationNames[getVocationProfile()]
end

-- Função para forçar auto-seleção
TargetBot.autoSelectProfile = function()
  autoSelectProfile()
end
