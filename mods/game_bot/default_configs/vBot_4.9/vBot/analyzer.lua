...
local function getPrice(name)
  ...
  -- custom price has priority
  local custom = storage.analyzers.customPrices[name]
  if custom then return custom end

  -- cache
  local cached = priceCache[name]
  if cached ~= nil then return cached end

  -- O(1) lookup from LootItems (dict by name), fallback 0
  local v = LootItems[name] or 0
  priceCache[name] = v
  return v
end
...
onTextMessage(function(mode, text)
    ...
    if not text:find("Loot of") and not text:find("The following items") then return end
    local name

    -- adding monster to killed list (incluindo bosses)
    local isBossLoot = text:find("The following items dropped by")
    local isNormalLoot = text:find("Loot of")
    
    if isNormalLoot then
      ...
      refreshKills()
    elseif isBossLoot then
      -- Extrai nome do boss
      name = text:match("The following items dropped by ([^%s]+(?:%s+[^%s]+)*) are available")
      if name and not killList[name] then
        killList[name] = 1
      elseif name then
        killList[name] = killList[name] + 1
      end
    ...
    if isNormalLoot then
        -- Processamento normal para criaturas
        split = string.split(text, ":")
        re = regexMatch(split[2], regex)
    elseif isBossLoot then
        -- Processamento especial para bosses
        local itemsText = text:match("reward chest: (.+)")
        if itemsText then
            -- Simula split para compatibilidade
            split = {"Loot of " .. (name or "Unknown"), itemsText}
            re = regexMatch(itemsText, regex)
        else
            return -- Se não conseguir extrair itens, sai
        end
    else
        return -- Se não for nem normal nem boss, sai
    end
    ...
    add(t, os.date("%H:%M") .. " Loot of " .. (name or "Unknown") .. ": ", "#FFFFFF", true)
    add(messageT, "Loot of " .. (name or "Unknown") .. ": ", "#FFFFFF", true)
    ...
    if re ~= 0 then
        for i=1,#re do
            ...
            -- ✅ LIMPEZA PERFEITA DE IDs - FORMATAÇÃO IGUAL AO TEXTMESSAGE
            local itemName = data
            local amount = 1
            
            if quantityMatch then
                amount = tonumber(quantityMatch)
                itemName = itemName:match("^%d+,%s*(.+)")
            end
            
            -- NOVO: extrair quantidade no formato "N nome" ou "N x nome" (ex.: "7 assassin stars", "3x demon horn")
            if amount == 1 then
                local q, rest = itemName:match("^%s*(%d+)[xX]?%s+(.+)$")
                -- evita confundir ID isolado (já é tratado mais abaixo)
                if q and rest and not rest:match("^%d+$") then
                    amount = tonumber(q)
                    itemName = rest
                end
            end
            
            -- Segundo: remover IDs que aparecem no formato "ID, nome" ou "nome, ID"
            itemName = itemName:gsub("^%d+,%s*", "")  -- Remove ID no início: "3035, platinum coins" -> "platinum coins"
            itemName = itemName:gsub(",%s*%d+$", "")  -- Remove ID no final: "platinum coins, 3035" -> "platinum coins"
            itemName = itemName:gsub(",%s*%d+,%s*", ", ")  -- Remove ID no meio: "item, 3035, item2" -> "item, item2"
            
            -- Terceiro: remover estruturas complexas {ID, {nome}, cor}
            itemName = itemName:gsub("{%d+,%s*{([^}]+)}[^}]*}", "%1")
            
            -- Quarto: remover códigos de cor e caracteres especiais
            itemName = itemName:gsub("#%w+", "")  -- Remove códigos de cor #FFFFFF
            itemName = itemName:gsub("[{}]", "")  -- Remove chaves
            
            -- Quinto: limpar espaços e vírgulas extras
            itemName = itemName:gsub("^[,%s]*", ""):gsub("[,%s]*$", "")  -- Remove vírgulas e espaços das bordas
            itemName = itemName:gsub("%s+", " ")  -- Remove espaços duplos
            
            -- ✅ DESCARTE: tokens que são apenas IDs (ex.: "7368", "3035" ou "a 7368")
            local raw = itemName
            -- normaliza espaços/artigos
            raw = raw:gsub("^%s+", ""):gsub("%s+$", "")
            -- casos: "7368" | "a 7368" | "an 7368"
            if raw:match("^%d+$") or raw:match("^[aA]n?%s*%d+$") then
                -- pula este "item" (não precifica, não soma valor, não adiciona ao texto)
                goto continue_item
            end
            
            -- Sexto: adicionar artigos quando necessário (igual textmessage)
            if amount == 1 and not itemName:match("^a%s") and not itemName:match("^an%s") and not itemName:match("^the%s") then
                if itemName:match("^[aeiouAEIOU]") then
            ...
            -- Calcular preço (remove artigos para busca)
            local searchName = itemName:gsub("^a%s+", ""):gsub("^an%s+", ""):gsub("^the%s+", "")
            local itemPrice = getPrice(searchName)
            local totalItemPrice = itemPrice * amount
            local color = getColor(totalItemPrice)
            
            combinedWorth = combinedWorth + totalItemPrice
            
            -- Atualizar lootedItems para Hunt Analyzer
            ...
            local itemId = stringToId(searchName)
            if not lootedItems[itemId] then
                lootedItems[itemId] = { count = amount, name = searchName }
            else
                lootedItems[itemId].count = lootedItems[itemId].count + amount
            end
            
            -- Formatar nome final EXATAMENTE como textmessage
            local displayName
            if amount > 1 then
                -- Para múltiplos: "5 platinum coins" (sem artigo)
                displayName = amount .. " " .. itemName:gsub("^a%s+", ""):gsub("^an%s+", ""):gsub("^the%s+", "")
            else
                -- Para único: "a platinum coin" (com artigo)
                displayName = itemName
            end

            add(t, displayName, color, i==#re)
            add(messageT, displayName, color, i==#re)
            ...
            for i, child in ipairs(dropTrackerWindow.contentsPanel:getChildren()) do
              ...
              -- ✅ CORREÇÃO: normalize para a "base" sem artigo e em minúsculas
              local baseName = itemName:gsub("^a%s+", ""):gsub("^an%s+", ""):gsub("^the%s+", ""):lower()
              if childName and baseName:find(childName:lower(), 1, true) then
            ...
            -- ✅ LABEL para permitir o continue (pular tokens de ID)
            ::continue_item::
        end
    end
...
local framesDirty, lastFramesUpdate = false, 0
local FRAMES_INTERVAL = 300 -- ms

local function setFrames()
  if g_game.getFeature(GameColorizedLootValue) then return end
  if not storage.analyzers.rarityFrames then return end
  if now - lastFramesUpdate < FRAMES_INTERVAL then return end
...
local function markFramesDirty() framesDirty = true end
macro(250, function() if framesDirty then setFrames() end end)

-- mark dirty on events
...
lootDirty = lootDirty or true
wasteDirty = wasteDirty or true
local lastLootWorth, lastWasteWorth, lastBalance = 0, 0, 0
function bottingStats()
  if lootDirty then
    lastLootWorth = 0
    for k, v in pairs(lootedItems) do
      local p = LootItems[v.name] or getPrice(v.name)
      lastLootWorth = lastLootWorth + (p * (v.count or 0))
    end
    lootDirty = false
  end
  if wasteDirty then
    ...
    for k, v in pairs(usedItems) do
      local p = LootItems[v.name] or getPrice(v.name)
      lastWasteWorth = lastWasteWorth + (p * (v.count or 0))
    end
    wasteDirty = false
  end
  lastBalance = lastLootWorth - lastWasteWorth
  return lastLootWorth, lastWasteWorth, lastBalance
end
