...
-- ========= SAFE HELPERS =========
-- Checagem segura de criatura no tile (cobre múltiplas APIs do OTC)
local function tileHasCreature(tile)
  if not tile then return false end
  if type(tile.hasCreature) == "function" then
    local ok, res = pcall(function() return tile:hasCreature() end)
  ...
  if type(tile.getTopCreature) == "function" then
    local ok, top = pcall(function() return tile:getTopCreature() end)
    if ok and top then return true end
  end
  return false
end
-- ===============================
...
onTextMessage(function(mode, text)
  ...
  for i, tile in ipairs(tiles) do
    local itemsCount = (type(tile.getItems) == "function" and #tile:getItems()) or 0
    if (not tileHasCreature(tile)) and tile:isWalkable() and itemsCount > 9 then
...
CaveBot.registerAction("goto", "green", function(value, retries, prev)
  ...
  if not path2 then
    ...
    for i, dir in ipairs(path) do
      ...
      if tile and tileHasCreature(tile) then
        local creatures = (type(tile.getCreatures) == "function" and tile:getCreatures()) or {}
        local creature = creatures[1]
        if creature then
          local hppc = (creature.getHealthPercent and creature:getHealthPercent()) or 0
          local ctype = (creature.getType and creature:getType()) or 0
          if creature.isMonster and creature:isMonster() and (hppc > 0) and (oldTibia or ctype < 3) then
