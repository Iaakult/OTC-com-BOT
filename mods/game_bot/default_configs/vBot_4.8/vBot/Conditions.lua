...
Panel
  ...
  macro(100, function()
    ...
    -- PRIORIDADE 1: Cura de paralisia (EMERGÊNCIA ABSOLUTA) - SEM DELAY
    if config.cureParalyse and mana() >= config.paralyseCost and isParalyzed() and 
       not getSpellCoolDown(config.paralyseSpell) then 
      say(config.paralyseSpell)
      registerSpellCast()
      return
    end
    
    -- PRIORIDADE 2: Utamo Vita - COOLDOWN FIXO DE 175 SEGUNDOS + delay entre magias
    if (not config.ignoreInPz or not isInPz()) and config.holdUtamo and mana() >= config.utamoCost and 
       (not utamoCast or (now - utamoCast > 175000)) and canCastSpell() then 
      say("utamo vita")
      utamoCast = now
      registerSpellCast()
      return
    ...
    -- PRIORIDADE 3: Haste - VERIFICAÇÃO IMEDIATA + delay entre magias
    if (not config.ignoreInPz or not isInPz()) and config.holdHaste and mana() >= config.hasteCost and 
       not hasHaste() and not getSpellCoolDown(config.hasteSpell) and canCastSpell() and
